...
```mermaid
flowchart LR
    C["CLEAN (DuckDB)<br/>main_clean.*"] -->|CleanReader| RD["dict por linha"]
...
2. **Lê a CLEAN** com o `CleanReader`
   ...
   in-process**: roda `SELECT ... FROM main_clean.<tabela>`, devolve **um dict por linha** em
   streaming (1000/vez), filtrando por watermark (`updated_at > X`).
...
> escreve *Cypher*, a linha da CLEAN vira `MERGE` independente de vir de `.duckdb`, Delta ou
> DuckLake. O gargalo de escala **não está no grafo** — está no extract do conector (§6).
