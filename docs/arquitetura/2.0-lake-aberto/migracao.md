# Migração para o lake aberto — código + runbook (agnóstico Delta / DuckLake)

> **Plano de migração da CLEAN do `.duckdb` para um lakehouse aberto** — vale para **Delta**
> **ou** **DuckLake** (a escolha é da [tarefa 01](tarefas/01-lakehouse/)). Onde diverge, o doc
> marca **[Delta]** / **[DuckLake]**. O caminho até o grafo **não muda** — o FalkorDB só fala
> Cypher sobre linhas; a migração toca só **escrita e leitura da CLEAN**.
>
> Comparação dos dois formatos: [tarefa 01](tarefas/01-lakehouse/). Base:
> [folder 05](../../../experimentacoes/05-formato-storage-lake/RESULTADOS.md). Índice:
...
Só a **CLEAN**. A RAW já é aberta (Delta). A CLEAN hoje são tabelas dentro do
`strattum.duckdb` (schema `main_clean`) → viram tabelas de lake:

```
strattum.duckdb::main_clean.dim_companies  ->  /data/clean/dim_companies   [Delta: 1 pasta]
strattum.duckdb::main_clean.fct_tickets     ->  /data/clean/fct_tickets     [ou tabela no catálogo DuckLake]
```
...
> [tarefa 01 §D](tarefas/01-lakehouse/).
...
> nesse ponto (ver [tarefa 01](tarefas/01-lakehouse/)).
